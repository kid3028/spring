...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		/**
		 * 分割name属性
		 * <bean id="testBean" class="test.alias"/>
		 * ---别名-->
		 * <bean id="testBean" name="testBean,testBean2" class="test.alias"/>
		 *
		 * or
		 *
		 * <bean id="testBean" class="test.alias"/>
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			// =====  lookup-method、replaced-method 都是构造一个MethodOverride，并最终记录在AbstractBeanDefinition的methodOverride
			// 解析 lookup-method
	 ...
	 *     <meta key="testStr" value="hhhh"/>
	 * </bean>
	 *
	 * meta不会体现在MyTestBean属性中，而是一个额外的声明，当需要使用里面的时候，可以通过BeanDefinition#getAttribute获取
	...
	/**
	 * <bean id="test" class="test.TestBeanProperty">
	 * 		<property name="testStr" value="foo"/>
	 * </bean>
	 *
	 * <bean id="a">
	 * 		<property name"b">
	...
	/**
	 * qualifier元素更多的通过注解形式来获取，在使用spring进行自动注入时，ioc容器中匹配的候选的bean数目
	 * 必须有且只有一个，当匹配不到bean是，spring抛出BeanCreationException，并指出必须只有有一个匹配的bean
	 * spring允许我们通过Qualifier指定注入的bean的名称
	 * <bean id="myTestBean" class="bean.MyTestBean">
	...
	/**
	 * 解析lookup-method属性
	 * lookup-method并不常用，但却有比较大作用，通常被称为  获取器注入
	 * 获取器注入是一种特殊的方法注入，他是把一个方法声明为返回某种类型的bean，但是实际要返回的bean是在配置文件里面配置的。
	 * 次方法在涉及有可插拔的功能，接触程序依赖
	 * <bean id="getBeanTest" class="test.lookup.GetBeanTest">
	 *     <lookup-method name="getBean" bean="teacher"/>
	 * </bean>
	 *
	 * <bean id="test" class="test.lookup.Teacher"/>
	 *
	 * public class User {
	 *     public void showMe() {
	 ...
	 * 与lookup不同，replaced-method不但可以动态替换返回实体bean，还能动态更改原有方法的逻辑
	 *
	 * public class TestChangedMethod {
	...
	/**
	 * 将index、type、name封装到  ValueHolder 中
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		// 要么ref，要么value，要么list
		NodeList nl = ele.getChildNodes();
		Element subElement = null;
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			// descriptor / meta 的不做处理
		...
		// constructor-arg不可能同时有ref属性又有value属性
		//  也不能ref + subEle or value + subEle
	 ...
	 * </bean>
	 *
	 * 当spring中的bean使用了默认的标签配置，但是子元素却使用了自定义的配置时，便会产生作用。
	...
	public BeanDefinitionHolder decorateBeanDefinitionIfRequired(Element ele, BeanDefinitionHolder originalDef) {
		// 第三个参数是父类bean，当对某个嵌套配置进行分析时，需要传递父类BeanDefinition，
		// 以备子类没有设置scope时默认使用父类的scope属性，这里分析的是顶层，所以传递null
	...
	/**
	 * 获取命名空间，然后与spring固定的命名空间比较，一致则是默认，否则自定义
